...
"""
The ones used here are not the same as in A. Bondi (1964). "van der Waals Volumes and Radii".
In the dmasif code they seem to use the correct ones.This is a crude version of RADII values as it maps all types 
of atoms begining with the same letter to the same radius.
"""
...
def msms_output_2numpy(file_base, biopython_version):
    """
    Args:
        file_base (string): The root of the msms output file.
        biopython_version (bool): If true, we can't have access to the full atom id.

    Returns:
        np.array : The vertices of the surface.
        np.array : The faces of the surface.
        np.array : The normals of the vertices of the surface.
    """
...
def pdb_to_xyzr(pdbfilename, xyzrfilename, ignore_HETATM=True):
    ...
    """
    Args:
        pdbfilename (string): The input name pdb file.
        xyzrfilename (string): The output name xyzr file.
        ignore_HETATM (bool): If true, ignore HETATM atoms from the input structure.
    """
...
it also adds and id to each atom which is probably unique, the color I'm not sure what it is for.
...
def generate_surface(pdb_path, args, complex_mesh_pth=None, selected_chains=None):
    """
    Generate the protein surface for a pdb structure. The surface is saved as a .ply file.
    For proteins that are part of a complex, we can also save the interface between the protein and the complex
    as an additional channel in the .ply file.

    Args:
        pdb_path (str): Path to the pdb file.
        args (dict): Dictionary of arguments.
            ignore_disordered (bool): Should disordered atoms be included or not.
            ignore_HETATM (bool): Should HETATM atoms be included or not.
            bioptyhon_version (bool): Use the biopython version of pdb_to_xyzr.
            probe_radius (float): Probe radius for MSMS.
        complex_mesh_pth (str): The path to the surface of the complex. Only needed if we want to store the interface.
        selected_chains (list): List of chains to be selected from the pdb file and used for the surface generation.
    """
...
def generate_surfaces_scop(args):
    ...
    """
    Generate surfaces for all SCOPe proteins. It can be sped up by running multiple processes in parallel.

    Args:
        args (dict): Dictionary of arguments.
            root (str): Path to the root directory containing the SCOPe pdb files (.ent).
            multi_process (bool): Should multiple processes be used or not.
    """
...
def generate_surfaces_ppi(ppi_file='../../data/masif-ppis/lists/all_ppi.txt',
                          ...
                          args=None):
    """
    Generate surfaces for all PPIs in the list. It can be sped up by running multiple processes in parallel.
    Args:
