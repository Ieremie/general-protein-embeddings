...
import pprint
from proemb.utils.hmm_util import get_combined_seq_hmm_probabilities, get_seq_hmm_probabilities_for_hsp
...
def main(args):
    ...
    hit_function = get_combined_seq_hmm_probabilities if args.full_hmm_augment_hits else get_seq_hmm_probabilities_for_hsp
    ...
    train_dataset = FlipDataset(base_pth=args.data_path, dataset=args.dataset, split=args.split,
                                ...
                                alphabet=alphabet, full_train=args.random_validation, softmax_tmp=args.softmax_tmp,
    ...
    if args.random_validation:
        ...
        val_dataset.augment_prob = 0
        val_dataset.noise_type = None
    else:
        val_dataset = FlipDataset(base_pth=args.data_path, dataset=args.dataset, split=args.split,  type='val',
                                  alphabet=alphabet)

    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, collate_fn=collate_seq_regression,
                              num_workers=8)
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, collate_fn=collate_seq_regression, num_workers=8)

    test_loader = DataLoader(
        FlipDataset(base_pth=args.data_path, dataset=args.dataset, split=args.split, type='test', alphabet=alphabet),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--softmax_tmp', type=float, default=1, help='softmax temperature for augment')
    parser.add_argument('--full_hmm_augment_hits', action='store_true',
                        help='if using the full hmm hits to augment instead of a single one')
