...
def train_info(start_time, nr_batches, loss_estimate, acc_estimate, i, output, writer, num_steps):
    epoch_mins, epoch_secs = epoch_time(start_time, time.time())
    print(f'Trained on {nr_batches} batches in: {epoch_mins}m {epoch_secs}s', file=output)

    line = '# [{}/{}] training {:.1%} loss={:.5f}, acc={:.5f}'
    line = line.format(i, num_steps, i / num_steps, loss_estimate, acc_estimate)
    print(line, file=output)

    writer.add_scalars('loss', {'train': loss_estimate}, i)
    writer.add_scalars('acc', {'train': acc_estimate}, i)
